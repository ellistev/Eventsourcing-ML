...
namespace Reservations.Api
{
    public class Startup : FunctionsStartup
    {
        ...
        private void RegisterRepositories(IFunctionsHostBuilder hostBuilder, IConfigurationRoot configRoot)
        {
            ...
            hostBuilder.Services.AddScoped<IRepository<RoomTemperatureViewsReadModel>>(x =>
            {
                var roomTypeAvailabilityCloudTable = GetTable(configRoot["ReservationsReadModels-TableStorage-ConnectionString"], configRoot["RoomTemperatureViewsReadModel-TableName"]);
                //set up cloudtables for each read model
                return new Repository<RoomTemperatureViewsReadModel>(roomTypeAvailabilityCloudTable);
            });
            
            hostBuilder.Services.AddScoped<IRepository<LocationWeatherReadModel>>(x =>
            {
                var roomTypeAvailabilityCloudTable = GetTable(configRoot["ReservationsReadModels-TableStorage-ConnectionString"], configRoot["LocationWeatherReadModel-TableName"]);
                //set up cloudtables for each read model
                return new Repository<LocationWeatherReadModel>(roomTypeAvailabilityCloudTable);
            });
        }
        ...
        private void RegisterReadModelDenormalizers(IBuilder builder)
        {
            ...
            new LocationWeatherDenormalizer(builder);
            new RoomViewsDenormalizer(builder);
            new RoomTemperatureViewsDenormalizer(builder);
